...
> **パッケージ管理**: uv  
> **CLI フレームワーク**: Click  
> **設定ファイル**: JSON (`~/.ai-adapter/config.json`)  
> **テスト**: unittest  
> **ライセンス**: MIT
...
ai_adapter.py add env X         →  config.json に env 名を保存
ai_adapter.py list              →  config.json から env 一覧を表示（* 付きでデフォルト環境）
ai_adapter.py del env X         →  config.json から env 名を削除（デフォルト環境は削除不可）
ai_adapter.py set-default X     →  config.json の default_env を更新
ai_adapter.py link-agent <A> <E> →  config.json の agent_bindings に追加
...
| `add env X` | `ai-adapter env add X` | `config.json` に env 名を保存 |
| `list` (env) | `ai-adapter env list` | env 一覧を表示（* 付きでデフォルト環境） |
| `del env X` | `ai-adapter env remove X` | `config.json` から env 名を削除（デフォルトは削除不可） |
...
│       ├── cli.py              # Click グループ定義
│       ├── config.py           # ConfigManager: ~/.ai-adapter/config.json の読み書き
...
```json
{
  ...
  "bins": [
    ...
    { "name": "deploy-staging.sh", "env": "myhome", "description": "ステージングデプロイ" },
...
各 dataclass に `to_dict()` / `from_dict()` メソッドを実装し、JSON とのシリアライズを可能にすること。
...
2. `config.json` が存在しなければ、デフォルト設定で初期化:
   - `default_env` を `"default"` に設定
...
2. `config.json` の `agents` にエントリを追加
3. ファイル名（拡張子除く）をエージェント名として使用

### `agent get <name>`

1. `config.json` からエージェント名を検索
...
2. `config.json` の `envs` に新しい Env を追加
...
3. `config.json` の `bins` にエントリを追加（env 名も記録）

### `bin get [env] <name>`

1. `[env]` が省略された場合は環境解決ロジックで補完
2. `config.json` から env + name でスクリプトを検索
...
3. `config.json` の `skills` にエントリを追加
...
2. `config.json` の `mcp_servers` に MCPServer を追加
3. `~/.ai-adapter/mcp/servers.json` にも設定を出力（バックアップ兼用）

### `mcp list`

1. `config.json` の `mcp_servers` 一覧をテーブル表示
...
1. `config.json` から MCP サーバーエントリを削除
2. 存在しなければエラー
...
- **`config.py`**: `~/.ai-adapter/config.json` の読み書き・バリデーション。
...
def init():
    ...
    # config.json が存在しなければデフォルト設定を書き込み
...
- リモートURLは `config.json` の `remote` フィールドに保存する案と、`git remote` で管理する案がある
...
### 13.7 MCP 設定の ai-adapter config.json 統合イメージ（将来）

```json
// ~/.ai-adapter/config.json（MCP セクション追加イメージ）
```json
{
  "version": 1,
  "mcp_servers": [
  ...
  "env_mcp_bindings": [
    { "env": "myhome", "mcp_servers": ["github", "filesystem"] },
    { "env": "office", "mcp_servers": ["github"] }
  ]
}
```
...
# env: config.json で管理（デフォルト環境 + エージェント紐付け）
add env NAME     →  config.json に env 名を追加
list             →  config.json の env 一覧を表示（* デフォルト環境）
del env NAME     →  config.json から env 名を削除（デフォルトは削除不可）
...
list skill           →  スキル一覧表示
del skill NAME       →  config.json から削除（--purge で実体も削除）
search skill KEYWORD →  スキルをキーワード検索
link skill A AGENT   →  スキル A をエージェント AGENT に紐付け

# mcp: config.json で管理（export で各ツール形式に出力）
mcp add NAME         →  config.json に MCP サーバー設定を追加
mcp list             →  MCP サーバー一覧表示（* 有効）
mcp del NAME         →  config.json から MCP サーバー設定を削除
